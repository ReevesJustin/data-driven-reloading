...
for trial_idx in range(N_SMALL_TRIALS):
    ...
    ax.plot(SEATING_DEPTHS[best_idx], group_sizes[best_idx], 'o',
            ...
            markeredgewidth=3, label=f'Apparent "sweet spot"')

    # Annotate the best
    ax.annotate(f'Best!\n{group_sizes[best_idx]:.2f} MOA',
                xy=(SEATING_DEPTHS[best_idx], group_sizes[best_idx]),
                xytext=(SEATING_DEPTHS[best_idx] + 0.005, group_sizes[best_idx] + 0.3),
...
fig.suptitle('Seating Depth Illusion: Small Samples Create False "Sweet Spots"\n'
             'Every Test Finds a Different "Best" - Proof It\'s Just Random Variation',
...
# Save the figure
output_path = Path(__file__).parent.parent / 'notebooks' / 'static' / 'nb07_plot13_seating_depth_scatter.png'
