...
gs = fig.add_gridspec(3, 2, hspace=0.35, wspace=0.3, height_ratios=[1, 1, 0.6])
...
summary_text = (
    ...
    f'    • Cost: ${cost_load}  |  Time: {time_load} hours\n'
    f'    • Precision improvement: {baseline_precision_moa - load_optimized_precision:.1f} MOA '
    f'({(baseline_precision_moa - load_optimized_precision)/baseline_precision_moa*100:.0f}%)\n'
    f'    • Hit probability increase: +{(load_hit_prob - baseline_hit_prob)*100:.0f}%\n\n'

    '  OPTION B - Improve Your Skills:\n'
    f'    • Cost: ${cost_skill}  |  Time: {time_skill} hours\n'
    f'    • Precision improvement: {baseline_precision_moa - skill_improved_precision:.1f} MOA '
    f'({(baseline_precision_moa - skill_improved_precision)/baseline_precision_moa*100:.0f}%)\n'
    f'    • Hit probability increase: +{(skill_hit_prob - baseline_hit_prob)*100:.0f}%\n\n'
