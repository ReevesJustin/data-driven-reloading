...
CHARGE_WEIGHTS = np.array([40.0, 40.5, 41.0, 41.5, 42.0, 42.5, 43.0])  # grains
...
for trial_idx in range(N_TRIALS):
    ...
    base_velocity = 2700  # fps at 40.0 grains
    true_velocities = base_velocity + TRUE_VELOCITY_SLOPE * (CHARGE_WEIGHTS - 40.0)
    ...
    for charge in CHARGE_WEIGHTS:
        true_vel = base_velocity + TRUE_VELOCITY_SLOPE * (charge - 40.0)
    ...
    # Detect and annotate apparent "flat spots" or "nodes"
    # Look for places where velocity change is less than expected
    velocity_changes = np.diff(measured_velocities)
    expected_change = TRUE_VELOCITY_SLOPE * 0.5  # 0.5 grain steps
    ...
    if len(flat_spots) > 0:
        ...
        ax.annotate('Apparent\n"node"?',
                    ...
                    xytext=(mid_charge + 0.5, mid_vel - 20),
...
explanation = (
    ...
    f'"nodes" or "flat spots" - proof that\n'
